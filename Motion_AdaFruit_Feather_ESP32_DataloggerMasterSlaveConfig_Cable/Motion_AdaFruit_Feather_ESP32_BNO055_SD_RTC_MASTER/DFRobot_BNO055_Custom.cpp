...
#include "DFRobot_BNO055_Custom.h"

void DFRobot_BNO055::SetChip1(void)
{
  address = BNO055_ADDRESS_CHIP1;
}
void DFRobot_BNO055::SetChip2(void)
{
  address = BNO055_ADDRESS_CHIP2;  
}


bool DFRobot_BNO055::initchip()
{
  uint16_t timeOut = 0;
  /*  IIC init */
  Wire.begin();
...
bool DFRobot_BNO055::init()
{
  ...
  if(!initchip())
  {
    Serial.println("Chip 1 init failed");
    return false;
  ...
  if(!initchip())
  {
    Serial.println("Chip 2 init failed");
    return false;
  }  
