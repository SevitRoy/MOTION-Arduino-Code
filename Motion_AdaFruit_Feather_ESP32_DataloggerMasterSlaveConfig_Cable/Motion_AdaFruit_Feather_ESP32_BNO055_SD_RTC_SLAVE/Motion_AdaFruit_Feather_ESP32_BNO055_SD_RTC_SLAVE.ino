 ...
 *  
 *  This code desribes the functionality of the slave datalogger, The BTSerial interface is removed, instead the device is triggered by listening on GPIO for a high singal of the master.
 *  Each time a high singal on this pin is detected, the slave starts a measurement.
 *  
 ...
 *  Note that before this code can be used the ESP32 core from espressiv needs to be installed, the driver for USB communication SiLabs CP2104 Needs to be installed on your PC, as well as the utilized librairies. 
   ...
   2019/Ockt/08 - Removed BT Serial librairy and functionality.
   2019/Ockt/08 - Added trigger on GPIO 27 to start a measurement.
...
void setup(void)
{
  ...
  display.setCursor(0,0);
  display.print("Slave module ready");

  display.setCursor(0,10);
  display.print("Waiting for trigger");
...
void loop() {
 ...
 /*if(clientConnected == true){
  ...
  if (digitalRead(SLAVE_TRIGGER) == HIGH){    // Waits for high input on pin SLAVE_TRIGGER.
